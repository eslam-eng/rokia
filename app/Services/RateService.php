...
class RateService extends BaseService
{
     ...
     */
    public function store(RateDTO $rateDTO): Model|Builder|null
    {
        $rateDTO->validate();
        $rateData = $rateDTO->toArray();
        return $this->getQuery()->create($rateData);
    }
     ...
     */
    public function update(RateDTO $rateDTO, int|Rate $rate): mixed
    {
        if (is_int($rate))
